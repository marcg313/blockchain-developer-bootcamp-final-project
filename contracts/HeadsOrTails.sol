...
contract HeadsOrTails {
  ...
  function lottery(uint8 guess) public payable returns(bool){
    ...
    return won; //Return value can only be used by other functions, but not within web3.js (as per latest version)
  }

  //Get the amount of games played so far
  function getGameCount() public view returns(uint) {
    return lastPlayedGames.length;
  }

  //Get statistics on previous games, e.g. address of player, amount bet, won or lost, and amount of ETH in the jackpot at current point in time
