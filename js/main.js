...
const contractAddress = "0xe4FbC73f03B5Ae1EAf0Aada6C9Cc596f66BBcCB6";//Contract address on Ropsten
let provider;
let signer;
let swissFranc;
...
async function loadWeb3() {
  ...
  if (window.ethereum) {
    try {
      ...
      showAlert("The App needs to access your account in order to play", "fail");
  ...
  // Non-dapp browsers...
    ...
    //Load blockchain and contract data (jackpot, last games) via ethers default provider (Infura, Etherscan)
    console.log('Non-Ethereum browser detected. Please connect with MetaMask to play!');
    showAlert("Non-Ethereum browser detected. Please connect MetaMask to play!", "fail");
    provider = ethers.getDefaultProvider('ropsten');
    // console.log(provider);
    // loadBlockchainData();
  }
...
async function loadBlockchainData() {
  //Show link to contract on Etherscan
  contractAddressShortened = contractAddress.slice(0, 4) + "..." + contractAddress.slice(-4);
  document.querySelector(".contract-address").innerHTML = '<a href="https://ropsten.etherscan.io/address/' + contractAddress + '">' + contractAddressShortened + '</a>';
  ...
  if (activeNetwork.chainId === deployedNetwork) {
    ...
    showAlert("Please switch to Ropsten testnet to play", "fail");
...
function three() {
  ...
  const textureCirc = new THREE.TextureLoader().load("img/coin.jpg");
  textureCirc.wrapS = THREE.RepeatWrapping;//repeat texture horizontally
  textureCirc.repeat.set(20, 0);//repeat 20x
  const textureHeads = new THREE.TextureLoader().load("img/heads.png");
  const textureTails = new THREE.TextureLoader().load("img/tails.png");
  const metalness = 0.7;
  const roughness = 0.3;
  ...
  //Update canvas on container size change.
